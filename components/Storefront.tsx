...
export function Storefront() {
  ...
  return <>
    ...
    <header className="site-header"><a className="brand" href="#top" aria-label="Fanny Cosméticos, início"><strong>Fanny</strong><span>COSMÉTICOS</span></a><nav><a href="#catalogo">Novidades</a><a href="#catalogo">Skincare</a><a href="#catalogo">Maquiagem</a><a href="#ritual">Seu ritual</a></nav><div className="account-actions" role="group" aria-label="Acesso à conta">{isAdmin && <button onClick={() => setAdminOpen(true)}>Administrar</button>}{signedIn ? <button onClick={() => { void supabase.auth.signOut(); }}>Sair</button> : <button onClick={() => setAccountOpen(true)}>Entrar</button>}</div><div className="header-actions"><label className="search-box"><span>⌕</span><input value={query} onChange={e => setQuery(e.target.value)} placeholder="Buscar seu cuidado" aria-label="Buscar produtos" /></label><button className="cart-button" onClick={() => setCartOpen(true)} aria-label={`Carrinho com ${count} itens`}>Sacola <span>{count}</span></button></div></header>
